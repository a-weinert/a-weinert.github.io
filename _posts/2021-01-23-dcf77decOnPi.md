...
```c
/*  Discrimination values for the 15% modulation time, shortened pulse. */
durDiscrPointData_t timDiscHs[5] = {
    {0,  30000, "spike", 's'}, // 0..30ms short modulation spike
    {1, 128999, "false", 'F'},
    {2, 130000, "undef", 'u'},
    {3, 228000, "true",  'T'},
    {4, MXUI32, "error", 'e'}      };
/*  Discrimination values for the modulation period, high grade receiver. */
 ...
 *  @return  the highest table entry with value >= table[i].v
 */
...
we get three good outcomes:   
  &nbsp; &bull; &nbsp; F.S &nbsp;false . second tick    
  &nbsp; &bull; &nbsp; T.S &nbsp;true&nbsp; . second tick   
  &nbsp; &bull; &nbsp; F.M &nbsp;false . end minute tick (2 seconds)
All other combinations are bad. Well, T.M per se would be good but is never
transmitted. Without the bad ones we had a chain of true and false only we 
could put indices respectively second numbers 0 .. 58 on we could decode.
...
30000 µs. After many experiments (over weeks) we keep it below 10ms.   
  &nbsp; &nbsp; testOnPi --glitch 9999 --DCF77 >> logs/dcf77test32cAnG.log &   
It came out that higher values might filter out many seconds periods in
sequence. Hence, one may wrongly detect no reception at all respectively 
receiver of. 

## An extra filter for modulation disturbances

The extra gain of a 30ms (maximum) glitch/spike filter compared with 10 ms
is marginal with good receivers. And anyway, the shortening of a
period below e.g. 400ms by a spike could not be handled by the glitch filter.
...
DCF77 1.844.065.532    28300  41: s#b   53930  14:17:40.135 ignor=
DCF77 1.844.119.462   174560  41: T=S  994862  14:17:41.087 -. 29 29.mm.
